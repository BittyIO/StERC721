    ...
    IStERC721Registry
{
    ...
    function createStERC721(
        address erc721,
        address stERC721Impl
    ) external override nonReentrant returns (address) {
        return _createStERC721(erc721, stERC721Impl);
    }

    function _createStERC721(
        address erc721,
        address stERC721Impl
    ) internal returns (address stERC721) {
        require(
            stERC721s[erc721] == address(0),
            "StERC721Registry: asset exist"
        );
        require(
            stERC721Impl != address(0),
            "StERC721Registry: impl is zero address"
        );
    ...
    function batchCreateStERC721(
        address[] calldata erc721s,
        address stERC721Impl
    ) external override nonReentrant returns (address[] memory stERC721s_) {
        for (uint256 i = 0; i < erc721s.length; i++) {
            stERC721s_[i] = _createStERC721(erc721s[i], stERC721Impl);
        }
    }

    function upgradeStERC721(
        address erc721,
        address stERC721Impl,
    ...
    function _upgradeStERC721(
        ...
        require(
            stERC721Proxy != address(0),
            "StERC721Registry: asset nonexist"
        );

        ITransparentUpgradeableProxy proxy = ITransparentUpgradeableProxy(
            payable(stERC721Proxy)
        );
        proxy.upgradeToAndCall(stERC721Impl, encodedCallData);
    }

    function batchUpgradeStERC721(
        address[] calldata erc721s,
        address stERC721Impl,
