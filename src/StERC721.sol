    ...
    ERC721EnumerableUpgradeable
{
    ...
    {
        return
            interfaceId == type(IStERC721).interfaceId ||
    ...
    function burn(uint256[] calldata tokenIds_) external override nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            IAssetVault assetVault_ = assetVaultRegistry.create(staker_);
            require(
                address(assetVault_) == _erc721.ownerOf(tokenId_),
                "stNft: invalid tokenId_"
            );
            _burn(tokenId_);
    ...
    function contractURI() external view override returns (string memory) {
        ...
        return
            string(
                abi.encodePacked("https://metadata.bitty.io/eth/", hexAddress)
            );
    }
    ...
    function getDelegateCashForTokenV2(
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            IAssetVault assetVault_ = assetVaultRegistry.get(
                ownerOf(tokenIds_[i])
            );
            if (address(assetVault_) == address(0)) {
                continue;
            }
            delegates[i] = assetVault_.getDelegateCashForTokenV2(
                address(_erc721),
                tokenIds_[i]
            );
        }
